...
### FTP port stuff
    ...
    for port in smb_port:
        if port in nmap_results.get('1.0', END):
                ...
                smtp_label = Label(ports_frame, text='SMTP is open').pack(anchor=W)
                ###add stuff to the tab
    ...
    if dns_port in nmap_results.get('1.0', END):
            ...
            dns_label = Label(ports_frame, text='Port 53 is open').pack(anchor=W)
            ###add stuff to the tab
    ...
    for port in http_port:
        if port in nmap_results.get('1.0', END):
                ...
                http_label = Label(ports_frame, text='HTTP is open').pack(anchor=W)
                ###add stuff to the tab
                threading.Thread(target=exec_nikto).start()
    ...
    for port in msrpc_port:
        if port in nmap_results.get('1.0', END):
                ...
                msrpc_label = Label(ports_frame, text='MSRPC is open').pack(anchor=W)
                ###add stuff to the tab
    ...
    for port in netbios_port:
        if port in nmap_results.get('1.0', END):
                ...
                netbios_label = Label(ports_frame, text='Netbios is open').pack(anchor=W)
                ###add stuff to the tab
    ...
    for port in smb_port:
        if port in nmap_results.get('1.0', END):
                ...
                smb_label = Label(ports_frame, text='SMB is open').pack(anchor=W)
                ###add stuff to the tab
                threading.Thread(target=exec_enum).start()  
        else:
            #ports_label = tkinter.Label(ports_frame, pady=50, text='There are no good ports to test')
...
## realtime output
    ...
    ssh_results = Text(ssh_frame, wrap='word', bg='Black', fg='Green')
    while True:
